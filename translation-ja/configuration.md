...
> {note} デプロイ時に config:cache を実行する場合、env 関数は設定ファイル内のみで利用し、それ以外の場所で呼び出していないことを、必ず確認してください。設定ファイルがキャッシュされた状態になると、.env ファイルのロードは行われず、また全ての env 関数の呼び出しがnullを返すようになります
